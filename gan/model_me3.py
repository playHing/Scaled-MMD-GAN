...
# -*- coding: utf-8 -*-
"""
Created on Mon Jul 31 15:22:03 2017
...
from model_mmd2 import DCGAN, tf, np
...
#                        [self.batch_size, self.output_size, self.output_size, self.c_dim],
            ...
            assert self.config.kernel in ['dot', 'mix_rq', 'mix_rbf'], \
            ...
            with tf.variable_scope('loss'):
                ...
                if 'full_gp' in self.config.suffix:
                    print('fullf gp case model_me2.py')
...
#        write_summary = True
        ...
        if self.d_counter == 0:
            ...
            if (np.mod(step, self.config.max_iteration//5) == 0):
